...
ai board [] = exhaust board board 0
ai board (x:xs) = 
       do
          let nimsum = foldr (+) 0 board          
          let target = (x^nimsum)
          let i = (length (x:xs))
          if target < x then do
          --let num = x-target
             if valid board i (x-target) && (x-target) > 0 then
                return (i, (x-target))
...
-- Mulig løsning
exhaust :: Board -> [Int] -> Int -> [(Int, Int)]
exhaust board (x:xs) i = 
            do
               let i = i+1
               if x > 0 then
                  return (i, x)
               else exhaust board xs i   
